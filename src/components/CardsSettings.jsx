...
export default function CardsSettings({searchParams}) {
  ...
  return (
    <Form onChange={handleFormChange} className="flex flex-col gap-2">
      <section className="w-full px-4 py-2 flex justify-between items-center gap-4 bg-white border rounded-lg">
        <input type="text" name="q" id="q" defaultValue={q} placeholder="Cari kartu..." className="p-0 flex-1 border-0 focus:ring-0" />
        <Search />
      </section>
      <section className="grid grid-cols-3 gap-2">
        <section className="p-2 flex flex-col justify-between gap-2 bg-white border rounded-lg relative">
        ...
        <section className="p-2 flex flex-col justify-center gap-2 bg-white border rounded-lg relative">
          <div className={`w-full flex justify-between items-center gap-2 ${q ? 'text-neutral-400' : ''}`}>
